...
def login(user, password):
    if user == 'Jai' and password == 'jai@123':  # Simple hardcoded login credentials
...
if not st.session_state['logged_in']:
    ...
    with st.sidebar:
        ...
        st.markdown("""
            <h4 style='text-align: left;' background-color:'>
    ...
    if predict_btn:
        ...
        with colb:
            ...
            st.markdown(f"""<h5 style='text-align: center;'>Statistics Summary about NPK and Weather Conditions values for <b> {rdf_predicted_value[0]} </b></h5>""", unsafe_allow_html=True)
